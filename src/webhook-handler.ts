...
function isWithin24h(timestamp: number): boolean {
  const now = Date.now();
...
async function handleInboundMessage(m: any) {
  ...
  const { rowCount } = await pool.query(
    ...
    ['instagram', 'inbound', sender, text, JSON.stringify({ mid, type: msg.type, ts }), mid],
  );
  if (rowCount === 0) {
