...
async function handleInboundMessage(m) {
    ...
    const { rowCount } = await pool.query('INSERT INTO mg_messages (channel, direction, external_user_id, text, meta, external_message_id) VALUES ($1, $2, $3, $4, $5, $6) ON CONFLICT (external_message_id) DO NOTHING', ['instagram', 'inbound', sender, text, JSON.stringify({ mid, type: msg.type }), mid]);
    if (rowCount === 0) {
    ...
    if (reply) {
        ...
        const ok = await (0, ig_api_1.igSendMessage)(sender, reply, token);
